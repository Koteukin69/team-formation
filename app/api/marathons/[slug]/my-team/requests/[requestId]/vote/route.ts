...
// POST /api/marathons/[slug]/my-team/requests/[requestId]/vote - Vote on request
  ...
  try {
    ...
    if (team.decisionSystem === 'dictatorship') {
      return NextResponse.json(
        { error: 'Голосование недоступно в режиме диктатуры' },
        { status: 403 }
      )
    }
    ...
    const alreadyVoted = teamRequest.votes.some(
      (v) => v.participantId.toString() === participant._id.toString()
    )
    ...
    const updatedRequest = await teamRequestsCollection.findOne({ _id: teamRequest._id })
    const approveCount = updatedRequest!.votes.filter((v) => v.vote === 'approve').length
    const rejectCount = updatedRequest!.votes.filter((v) => v.vote === 'reject').length
    const majority = Math.floor(team.memberCount / 2) + 1
...
// Process approved request (same as in route.ts)
