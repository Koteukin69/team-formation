...
// POST /api/marathons/[slug]/participants/[id]/ban - Ban participant
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (participant.userId.toString() === marathon.creatorId.toString()) {
      return NextResponse.json(
        { error: 'Нельзя забанить создателя марафона' },
        { status: 403 }
      )
    }

    // Cannot ban other organizers (only creator can remove organizers)
    ...
    if (isTargetOrganizer && !isCreator) {
      return NextResponse.json(
        { error: 'Нельзя забанить организатора' },
        { status: 403 }
      )
    }
    ...
    if (participant.teamId) {
      ...
      if (team) {
        ...
        if (newMemberCount === 0) {
          ...
          // Check if banned member was leader
